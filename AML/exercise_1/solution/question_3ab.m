function question_3ab
    ...
    for max_k=k_values
        ...
        axis([0 1 -2 2]);
        title(strcat('Model with k : ', num2str(max_k))); 
        pause
    end
    hold off;
    ...
    plot(repmat(k_values,size(train_err,2),1)', train_err', 'r');
    hold off
    figure(6) ; title('Test Err'); hold on
    plot(repmat(k_values,size(test_err,2),1)', test_err', 'g');
    hold off;
